...
from Segmenter import searchcriminal
def spider(request):
    text = ""
    context = {}
    if request.GET:
        ...
        have = Datatext.objects.all().filter(Criminals=str(request.GET['key']))
        print(have)
        #input("continue..")
        context['alltext'] = []
        for val in have:
        ...
        text = searchcriminal.getData(request.GET['key'], int(request.GET['number']), str(request.GET['start']),
        ...
        for var in list:
            if len(var) != 0 and str(request.GET['key']) in var:
                Datatext.objects.get_or_create(Content=var, Criminals=str(request.GET['key']), Unique=var[0:199])
                context['alltext'].append(var)
                count += 1
